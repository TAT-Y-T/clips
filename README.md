...
```bash
python3 clips.py /data/all_sessions --output-dir /data/clips_output
```
...
```bash
python3 clips.py /data/session_001 --no-validate-mp4-frames
```
...
```bash
python3 clips.py /data/session_001 --dry-run
```

从 `upload/` 读取视频：

```bash
python3 clips.py /data/session_001 --input-subdir upload
```

使用另一组时间戳：

```bash
python3 clips.py /data/session_001 --timeline decoded
```

强制重建所有输出：

```bash
python3 clips.py /data/session_001 --force --reencode
```

关闭计时输出：

```bash
python3 clips.py /data/session_001 --no-timer
```
